 ...
 * MOD 16-4L Modify to include a price componant
 */
...
void insert(struct part inventory[MAX_PARTS], int *num_parts){
    ...
    if(*num_parts == 0 || part_number > inventory[*num_parts - 1].number){
...
void updatePrice(struct part inventory[MAX_PARTS, int num_parts]){
    int i, number;
