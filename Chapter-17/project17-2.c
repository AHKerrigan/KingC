 ...
 * MOD 17-1 - Modify so that the inventory array is allocated dynamically
 * MOD 17-2 - Modify so that print sorts the array with qsort first
 * before printing.
 */
...
void insert(void){
    ...
    if(num_parts == arraySize){
        arraySize *= 2;
        inventory = realloc(inventory, sizeof(struct part) * arraySize);
    }
...
void print(void){
    int i;
    qsort(inventory, num_parts, sizeof(struct part), compareParts);
...
int main(void){
    char code;
    inventory = malloc(sizeof(struct part) * arraySize);
